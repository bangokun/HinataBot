...
END:VCARD`.trim()
    ...
    */
    ...
    let file = await conn.resize(buf, 300, 150)
    let a = [1,2]
    ...
    */
    ...
    async value(jid, title, text, footer, buttonText, buffer, listSections, quoted, options) {
         ...
         if (buffer && !Buffer.isBuffer(buffer) && (typeof buffer === 'string' || Array.isArray(buffer))) (options = quoted, quoted = listSections, listSections = buffer, buffer = await conn.resize(buffer, 300, 150))
         if (!options) options = {}
         // send a list message!
    ...
    */
    ...
    async value(jid, buffer, content, footerText, button1, id1, button2, id2, button3, id3, quoted, options) {
        ...
        const buttons = [
            { index: 1, urlButton: { displayText: htjava + ' My Github', url: sgh } },
    ...
    async value(jid, buffer, content, footerText, button1, id1, quoted, options) {
        const buttons = [
            { index: 1, urlButton: { displayText: htjava + ' My Github', url: sgh } },
            { index: 2, callButton: { displayText: htjava + ' My Owner',  phoneNumber: nomorown } },
    ...
    /* */
        ...
        sendButton: {
            ...
            async value(jid, text = '', footer = '', buffer, buttons, quoted, options) {
                let type
                if (Array.isArray(buffer)) (options = quoted, quoted = buttons, buttons = buffer, buffer = await conn.resize(buffer, 300, 150))
                else if (buffer) try { (type = await conn.getFile(buffer), buffer = type.data) } catch { buffer = await conn.resize(buffer, 300, 150) }
                ...
                let message = {
                    ...
                    ...(buffer ?
                        options.asLocation && /image/.test(type.mime) ? {
                            ...
                            [/video/.test(type.mime) ? 'video' : /image/.test(type.mime) ? 'image' : 'document']: type
                        } : {})
                }
        ...
        sendHydrated: {
            ...
            async value(jid, text = '', footer = '', buffer, url, urlText, call, callText, buttons, quoted, options) {
                let type
                if (buffer) try { (type = await conn.getFile(buffer), buffer = type.data) } catch { buffer = buffer }
                if (buffer && !Buffer.isBuffer(buffer) && (typeof buffer === 'string' || Array.isArray(buffer))) (options = quoted, quoted = buttons, buttons = callText, callText = call, call = urlText, urlText = url, url = buffer, buffer = await conn.resize(buffer, 300, 150))
                if (!options) options = {}
                let templateButtons = []
                if (url || urlText) {
