...
END:VCARD`.trim()
    ...
    */
    ...
    async value(jid, title, text, footer, buttonText, buffer, listSections, quoted, options) {
         ...
         if (buffer && !Buffer.isBuffer(buffer) && (typeof buffer === 'string' || Array.isArray(buffer))) (options = quoted, quoted = listSections, listSections = buffer, buffer = await conn.resize(type, 300, 150))
         if (!options) options = {}
         // send a list message!
    ...
    */
    ...
    async value(jid, buffer, content, footer, button1, row1, quoted, options = {}) {
        let type = await conn.getFile(buffer)
        let { res, data: file } = await conn.resize(type, 300, 150)
        if (res && res.status !== 200 || file.length <= 65536) {
        try { throw { json: JSON.parse(file.toString()) } }
        catch (e) { if (e.json) throw e.json }
        }
    ...
    /* */
        ...
        sendButton: {
            ...
            async value(jid, text = '', footer = '', buffer, buttons, quoted, options) {
                let type
                if (Array.isArray(buffer)) (options = quoted, quoted = buttons, buttons = buffer, buffer = await conn.resize(type, 300, 150))
                else if (buffer) try { (type = await conn.getFile(buffer), buffer = type.data) } catch { buffer = await conn.resize(type, 300, 150) }
        ...
        sendHydrated: {
            ...
            async value(jid, text = '', footer = '', buffer, url, urlText, call, callText, buttons, quoted, options) {
                let type
                if (buffer) try { (type = await conn.getFile(buffer), buffer = type.data) } catch { buffer = buffer }
                if (buffer && !Buffer.isBuffer(buffer) && (typeof buffer === 'string' || Array.isArray(buffer))) (options = quoted, quoted = buttons, buttons = callText, callText = call, call = urlText, urlText = url, url = buffer, buffer = await conn.resize(type, 300, 150))
                if (!options) options = {}
                let templateButtons = []
                if (url || urlText) {
